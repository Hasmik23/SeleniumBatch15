...
////        check the checkbox that contains money in the table on the website
//        check the checkbox that contains money
        for(int i=0;i<column.size();i++){
//            text
            String text = column.get(i).getText();
            if(text.equalsIgnoreCase("MyMoney")){  //I change to familyAlbum and will still work if(text.equalsIgnoreCase("FamilyAlbum")){
               // System.out.println("money is in the list on index"+i);   // tell me the row number of money in this list
                System.out.println("money is in the table on index"+(i+2));  //+ concatination t be operational
                driver.findElement(By.xpath("//table[@class='SampleTable']/tbody/tr["+(i+2)+"]/td[1]")).click();  //checkbox
